...
See [Relationships → Introspection](../guides/feature-relationships.md#introspection).
...
The same modes can be set on a single collection, which takes precedence over
the class setting and the `Familia.dirty_write_warnings` global:

```ruby
Familia::HashKey.new('bookkeeping', parent: record, dirty_write_warnings: :off)
```

This exists for ORM-internal structures that the persistence path writes
deliberately while the parent is dirty — the instance-scoped index tracker uses
it, since "parent has unsaved scalar fields" is false there (the scalar write is
queued in the same transaction) and `Familia.strict_write_order` would otherwise
abort the save. Prefer the class or global setting for your own data. An
unrecognized mode raises `ArgumentError` at construction.
