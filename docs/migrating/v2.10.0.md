...
To avoid enumerating every index by hand, sweep them project-wide. The
introspection layer detects which class-level unique indexes still hold legacy
data and rebuilds them — without the calling code knowing any index internals:

```ruby
# Rebuild only the indexes that still hold pre-2.10.0 data:
Familia.stale_indexes.each(&:rebuild!)
```

Add a boot guard (or CI smoke test) so an un-rebuilt index is caught up front
instead of silently failing a `find_by_*` lookup at runtime:

```ruby
Familia.assert_indexes_current!                  # raises if any index is stale
Familia.assert_indexes_current!(on_stale: :warn) # or just warn
```
