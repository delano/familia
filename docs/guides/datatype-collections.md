...
An instance created before a late declaration still gets the field. Its accessor finds no built collection, builds that one field from the current definition (so a `configure_related_field` that ran between the declaration and this access applies), freezes that definition, and caches the result on the instance. The build is single-flight under the class's build lock, so concurrent first access constructs it once, and fields built at initialization are never rebuilt. `update_expiration`, `persist!`, `ttl_report`, and instance `destroy!` walk the current registry and so cover the late field on old instances without recreating them. One gap: a subclass copies the registry when it is defined, so a field declared on the base class after that has an inherited accessor but no definition in the subclass; calling it raises `Familia::HorreumError` naming the problem.
