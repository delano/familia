# try/validation/performance_validation_try.rb
...
commands = validator.capture_redis_commands do
  ...
  user.save
  user.activities.lpush("activity1", "activity2", "activity3")
  user.tags.sadd("tag1", "tag2")
  user.scores.zadd(100, "score1")
end
...
commands = validator.capture_redis_commands do
  ...
  user.activities.lpush("item")  # List operation
  user.tags.sadd("tag")  # Set operation
  user.scores.zadd(50, "score")  # Sorted set operation
end
...
assert_efficient_commands do
  ...
  user.save
  user.activities.lpush("single_activity")
end
#=> true
...
commands = validator.capture_redis_commands do
  ...
  (1..10).each { |i| user.activities.lpush("item#{i}") }
end
...
empty_commands = CommandRecorder::CommandSequence.new
analysis = PerformanceAnalyzer.new(empty_commands).analyze
...
commands = validator.capture_redis_commands do
  ...
  user.save
  user.activities.lpush("test_activity")
...
[
  ...
  commands.transaction_count == 1,
  commands.transaction_blocks.first.command_count == 2
]
#=> [true, true, true]
...
first_command.duration_us >= 0 && first_command.timestamp.is_a?(Time)
#=> true
...
commands = validator.capture_redis_commands do
  ...
  user1.activities.lpush("logged_in")
  user2.activities.lpush("logged_in")
end
...
[
  commands.transaction_count == 1,
  commands.transaction_blocks.first.command_count == 3,
  atomic_commands.length == 3,
  non_atomic_commands.length > 3
]
#=> [true, true, true, true]
